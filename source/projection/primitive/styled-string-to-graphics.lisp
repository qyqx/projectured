                         ...
                         (etypecase element
                           (styled-string/string
                            (bind ((content (content-of element))
                                   (styled-string-reference `(content-of (the styled-string/string (elt (the list (elements-of ,typed-input-reference)) ,input-index)))))
    ...
    (cond ((and (typep latest-gesture 'gesture/keyboard/key-press)
           ...
           (make-operation/replace-selection document
                                             (ecase (key-of latest-gesture)
                                               (:sdl-key-home
                                                ...
                                                `(the sequence-position (pos (the string (content-of (the styled-string/string (elt (the list (elements-of (the styled-string/document (content-of (the document document))))) 0)))) 0)))
                                               (:sdl-key-end
                                                `(the sequence-position (pos (the string (content-of (the styled-string/string (elt (the list (elements-of (the styled-string/document (content-of (the document document)))))
          ...
          ((and (typep latest-gesture 'gesture/keyboard/key-press)
           ...
           (pattern-case (selection-of document)
             ((the sequence-position (pos (the string (content-of (the styled-string/string (elt (the list (elements-of (the styled-string/document ?a))) ?element-index)))) ?character-index))
              (bind ((string (styled-string/as-string input))
                     (character-index (styled-string/index input ?element-index ?character-index))
                ...
                (bind ((character-index (iter (with line-begin-index = 0)
                       ...
                       (selection `(the sequence-position (pos (the string (content-of (the styled-string/string (elt (the list (elements-of (the styled-string/document ,?a)))
                                                                                                                      ,(styled-string/element-index input character-index)))))
                                                               ,(styled-string/character-index input character-index)))))
          ...
          ((key-press? latest-gesture :key :sdl-key-delete)
           (pattern-case (selection-of document)
             ((the sequence-position (pos (the string ?a) ?b))
              (when (< ?b (styled-string/length (content-of document)))
